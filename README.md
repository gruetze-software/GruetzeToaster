...
- Modern Customization: * Toaster Count: From a single scout to a chaotic breakfast swarm.
- Animation Speed: Fine-tune the velocity of your flying appliances.
- Native Windows Integration: Fully compatible with the Windows Screensaver protocol, including:
- Mini-Preview: Works inside the small Windows monitor icon.
  Settings Dialog: Native configuration window.
  Fullscreen Mode: Smooth execution across all monitors.
- Cross-Platform: Running under Windows, Linux and MacOS. There is a suitable release for each system.
